...
const diceRoll = (array) => {
  let diceNumber = Math.ceil(Math.random() * array.length);
  return diceNumber;
}
...
playBtn.addEventListener('click', () => {
  ...
  // 4) Create two variables to store player one and player two scores
    ...
    let playerOneScore = diceRoll(twelveSidedDie);
    let playerTwoScore = diceRoll(twelveSidedDie);
  ...
  const scoreOneElement = document.querySelector(playerOneScore);
  const scoreTwoElement = document.querySelector(playerTwoScore);
